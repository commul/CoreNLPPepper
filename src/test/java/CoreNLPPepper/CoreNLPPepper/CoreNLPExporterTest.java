...
import CoreNLPPepper.CoreNLPPepper.CoreNLPExporter;
 ...
 * {@link CoreNLPExporter} class. Feel free to adapt and enhance this test class
 ...
 * @author Okinina
 */
public class CoreNLPExporterTest extends PepperExporterTest {
	/**
	...
	@Before
	public void setUp() {
		setFixture(new CoreNLPExporter());
	...
	public void test_DummyImplementation() {
		...
		getFixture().setCorpusDesc(new CorpusDesc().setCorpusPath(URI.createFileURI(getTempPath("CoreNLPExporter").getAbsolutePath())));

		// starts the Pepper framework and the conversion process
		start();

		File superCorpus = new File(getTempPath("CoreNLPExporter").getAbsolutePath() + "/rootCorpus");
